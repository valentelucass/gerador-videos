...
- **2026-08-01:** `automation/.env` aceita `RESUME_URL` para uma URL Vibes de projeto/conteúdo validada pelo operador. A retomada usa essa URL, considera o upload já concluído para a seleção atual e entra diretamente na fase de animação; não recria projeto nem envia arquivos.
...
- **2026-08-02:** os checkpoints da automação Vibes agora são separados por `project_id` do painel em `automation/state/projects/`. O botão `Animar IA` nunca consulta `RESUME_URL`: ele começa/continua apenas o projeto local atual. A URL externa só é usada pelo botão explícito `Retomar Vibes`, impedindo que um projeto novo herde a página, os uploads ou o progresso de outro.
- **2026-08-02:** a seleção sequencial do Vibes aceita a coluna lateral mesmo quando ela não tem overflow físico. Lotes pequenos podem mostrar todos os thumbnails de uma vez; isso é uma lista válida, não um erro que justifique refresh. O robô só rola quando houver conteúdo fora da viewport.
...
- **2026-08-02:** a entrega horizontal foi elevada para 1080p60. O compositor usa a RX 7600 com AMF em `high_quality`/`quality`, perfil H.264 High e CQP 18/20/22; recursos analíticos instáveis do driver foram removidos posteriormente. Isso aumenta substancialmente custo de GPU, tempo e espaço temporário, mas preserva melhor detalhes antes da recompressão de plataformas; fontes de 24/30 fps não recebem movimento nativo novo.
- **2026-08-02:** a grade de 60 fps preserva a cadência editorial anterior: transições gerais usam 24 quadros (0,40 s) e transições de cartões 44 quadros (~0,73 s). O renderer também diferencia a interrupção externa do FFmpeg (`-1`/`4294967295` no Windows) de uma falha real de composição e a expõe ao painel como `render_interrupted`.
- **2026-08-02:** após um job 1080p60 ficar preso ao abrir duas sessões AMF simultâneas, a composição horizontal passou a serializar cenas e segmentos (`SCENE_RENDER_WORKERS=1`). O painel agora exibe uma estimativa dinâmica de tempo restante, calculada pela velocidade real do próprio job depois de 45 s e recalibrada a cada atualização de progresso.
- **2026-08-02:** o AMF da RX 7600 encerrou o processo com `0xC0000409` durante cenas comuns, mesmo com uma única sessão. Recursos analíticos do driver (`preanalysis`, boost de alto movimento e VBAQ) foram removidos; permanecem `high_quality`, perfil High e CQP 18/20/22. Cada clipe curto possui watchdog de 60 s: no primeiro crash/timeout AMF, ele e todas as cenas restantes do job passam automaticamente para `libx264` CRF 17/preset fast, sem abortar a renderização inteira.
- **2026-08-02:** o fallback `libx264` compõe até dois cartões independentes em paralelo; AMF continua sempre serial. Benchmark real de dois cartões 1080p60/7,67 s validou 37,09 s em paralelo contra 56,47 s em série, com H.264 1920×1080/60 fps e duração idênticos. A regra reduz o gargalo de CPU sem abrir duas sessões AMD.
- **2026-08-02:** foi criado o prompt independente `PROMPT_JSON_ROTEIRO_GATOS_SEM_BROLL.md`, exposto no painel como “Prompt gatos”. Ele usa somente imagens, a estrutura de cinco fases para comportamento felino e o marcador `ilustração felina editorial`; o exportador do Google Flow reconhece o marcador e aplica automaticamente o preset de ilustração clara com gato/tutor recorrentes, sem B-roll.
...
- **2026-08-02:** `automation/prompts/animate.md` agora inclui modo estrito para ilustrações 2D. Desenhos, cartoons, line art e artes editoriais preservam traço, camadas planas, forma e perspectiva; não recebem parallax/câmera 3D, volumetria, CGI, textura fotográfica ou reluzimento dimensional. O movimento permitido nesses casos é uma animação 2D contida e coerente com o desenho.
- **2026-08-02:** o bloqueio 2D do `animate.md` foi elevado a regra inicial e prioritária: para qualquer imagem estilizada, o pedido declara animação — não recriação — e exige tratar dúvidas como 2D. As permissões de câmera e parallax agora são exclusivas de fontes claramente fotográficas; em desenho, fundo e sujeito não podem mover-se em velocidades de profundidade diferentes.
