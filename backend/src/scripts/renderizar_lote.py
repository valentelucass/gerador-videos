...
from backend.src.utils.file_retry import rmtree_com_retry
...
OUTPUT_DIR = ROOT_DIR / "workspace" / "output" / "vertical"
    ...
    *,
    ...
    for metadata_path in metadata_files:
        ...
        try:
            ...
            if limpar_artefatos:
                _limpar_lote_vertical_concluido(tema_dir, destino_final)
            resultados.append(
                {
                    **contexto,
                    "metadata": str(metadata_path),
...
def _limpar_lote_vertical_concluido(tema_dir: Path, output_path: Path) -> None:
    """Descarta o lote curado somente depois de instalar o MP4 final."""

    lote = tema_dir.resolve()
    raiz = LOTES_PREPARADOS_DIR.resolve()
    final = output_path.resolve()
    try:
        relativo = lote.relative_to(raiz)
    except ValueError as exc:
        raise RuntimeError(f"Recusa ao limpar lote fora de lotes_preparados: {lote}") from exc

    if len(relativo.parts) < 2 or not lote.is_dir():
        raise RuntimeError(f"Diretorio de lote vertical inseguro para limpeza: {lote}")
    if not final.is_file() or final.stat().st_size <= 0:
        raise RuntimeError(f"Resultado final ausente ou vazio; lote preservado: {final}")
    try:
        final.relative_to(lote)
    except ValueError:
        pass
    else:
        raise RuntimeError(f"Recusa ao apagar lote que contem o resultado final: {lote}")

    rmtree_com_retry(lote)
