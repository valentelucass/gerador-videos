...
from backend.src.utils.file_retry import replace_com_retry, unlink_com_retry
...
class EstadoLocalT12(TypedDict):
    buscas: set[str]
    midias: set[Path]


class EstadoT12(TypedDict):
    local: dict[str, EstadoLocalT12]
    pexels_ids: dict[str, set[str]]
    pexels_tokens: dict[str, list[str]]
...
def preparar_horizontal(
    ...
    *,
    ...
    midias_locais_prioritarias = _midias_locais_do_manifesto()
    diversificar_templates = _diversificar_templates_do_manifesto()
    ...
    for tema in temas:
        resultado = _preparar_tema(
            ...
            fetcher=pexels,
            midias_locais_prioritarias=midias_locais_prioritarias,
            diversificar_templates=diversificar_templates,
            logger=logger,
        )
...
def _preparar_tema(
    *,
    ...
    logger: Any,
    midias_locais_prioritarias: tuple[Path, ...] = (),
    diversificar_templates: bool = False,
) -> dict[str, Any]:
    ...
    if diversificar_templates:
        _diversificar_templates_consecutivos(metadata["cenas"], logger)
...
def _preparar_midias_do_tema(
    *,
    ...
    total_pexels = 0
    total_prompts = 0
    midias_locais_projeto_usadas: set[Path] = set()

    for indice_cena, cena in enumerate(cenas, start=1):
        slots = _slots_da_cena(cena)
        midias_locais_usadas: set[Path] = set()
        grupo_t12 = _grupo_t12(cena)
        for slot in slots:
            if slot.fonte_midia == "pexels":
                origem_local = _escolher_midia_local_prioritaria(
                    midias_locais_prioritarias,
                    midias_locais_projeto_usadas,
                )
                if origem_local is not None:
                    midias_locais_projeto_usadas.add(origem_local.resolve())
                    destino_final = destino / _nome_arquivo_local(indice_cena, slot.letra, origem_local.suffix)
                    _copiar_midia_local(origem_local, destino_final)
                    _marcar_slot_como_local(cena, slot.indice - 1)
                    logger.info("Horizontal: cena %s slot %s usou mídia local prioritária '%s'.", indice_cena, slot.letra or "-", origem_local.name)
                    continue
                busca = _exigir_texto_slot(slot, "Pexels")
                busca = _diversificar_busca_pexels_t12(
                    busca=busca,
                    cena=cena,
                    indice_cena=indice_cena,
                    estado_t12=estado_t12,
                    logger=logger,
                )
                ids_grupo = _ids_pexels_t12(estado_t12, grupo_t12)
                # PexelsFetcher seleciona o primeiro candidato que nao esteja
                # neste conjunto. Ao incluir os IDs do grupo antes da chamada,
                # ele pula o resultado repetido e usa o proximo item da mesma
                # resposta da API, sem um request adicional.
                media = fetcher.obter_midia_para_cena(
                    query=busca,
                    midias_usadas=midias_usadas | ids_grupo,
                    storage_path=destino,
                    orientacao="landscape",
                )
                media_id = str(media.get("id", "")).strip()
                if not media_id:
                    raise PexelsFetcherError(
                        f"cena {indice_cena}: Pexels nao retornou um ID de midia valido."
                    )
                if media_id in ids_grupo:
                    raise PexelsFetcherError(
                        f"cena {indice_cena}: Pexels retornou o ID {media_id} ja usado no "
                        "mesmo grupo T12 apesar da exclusao de candidatos."
                    )
                midias_usadas.add(media_id)
                if grupo_t12 is not None:
                    ids_grupo.add(media_id)
                    _prompts_pexels_t12(estado_t12, grupo_t12).append(busca)
            ...
            if slot.fonte_midia == "local":
                try:
                    if grupo_t12 is None:
                        ...
                        origem = _resolver_midia_local_t12(
                            diretorio_origem=diretorio_origem,
                            busca_local=slot.prompt_ou_busca,
                            grupo_t12=grupo_t12,
                            indice_cena=indice_cena,
                    ...
                    origem_prioritaria = _escolher_midia_local_prioritaria(
                        midias_locais_prioritarias,
                        midias_locais_projeto_usadas,
                    )
                    if origem_prioritaria is not None:
                        origem = origem_prioritaria
                        midias_locais_projeto_usadas.add(origem.resolve())
                        _marcar_slot_como_local(cena, slot.indice - 1)
                    else:
                        prompt_flow = _prompt_google_flow_por_slot(cena, slot.indice - 1)
                        if not prompt_flow:
                            raise
                        prompt_path = destino / _nome_arquivo_prompt_google_flow(indice_cena, slot.letra)
                        prompt_path.write_text(prompt_flow.rstrip() + "\n", encoding="utf-8")
                        logger.warning(
                            "Horizontal: cena %s slot %s aguarda mídia local '%s'. Instrução Google Flow salva em %s. %s",
                            indice_cena,
                            slot.letra or "-",
                            slot.prompt_ou_busca or "sem tag",
                            prompt_path.name,
                            exc,
                        )
                        continue
...
STOPWORDS_LEXICAS_INGLES = frozenset(
    {"the", "a", "an", "in", "on", "at", "with", "and", "of", "for"}
)
LIMIAR_SIMILARIDADE_PEXELS_T12 = 0.60


def _grupo_t12(cena: dict[str, Any]) -> str | None:
    grupo = cena.get("t12_group_id")
    if not isinstance(grupo, str) or not grupo.strip():
        return None
    return grupo.strip()


def _estado_local_t12(
    estado_t12: EstadoT12,
    grupo_t12: str,
) -> EstadoLocalT12:
    local = estado_t12["local"].setdefault(
        grupo_t12,
        {"buscas": set(), "midias": set()},
    )
    return local
...
def _ids_pexels_t12(
    ...
    if grupo_t12 is None:
        return set()
    ids = estado_t12["pexels_ids"].setdefault(grupo_t12, set())
    return ids


def _prompts_pexels_t12(
    estado_t12: EstadoT12,
    grupo_t12: str,
) -> list[str]:
    prompts = estado_t12["pexels_tokens"].setdefault(grupo_t12, [])
    return prompts


def _resolver_midia_local_t12(
    *,
    diretorio_origem: Path,
    ...
    """Resolve uma midia local sem reutilizar assets dentro de um grupo T12."""

    busca = _normalizar_busca_local(busca_local)
    estado_local = _estado_local_t12(estado_t12, grupo_t12)
    buscas_usadas = estado_local["buscas"]
    midias_usadas = estado_local["midias"]
    chave_busca = _chave_busca_local(busca)

    if chave_busca in buscas_usadas:
        logger.warning(
            "Horizontal: cena %s T12 grupo=%s repetiu busca_local '%s'; "
            "selecionando asset local alternativo.",
            indice_cena,
            grupo_t12,
            busca,
        )
        origem = _sortear_alternativa_local_para_busca(
            diretorio_origem,
            busca,
            midias_usadas,
            grupo_t12,
    ...
    buscas_usadas.add(chave_busca)
    midias_usadas.add(origem.resolve())
    return origem


def _sortear_alternativa_local_para_busca(
    diretorio_origem: Path,
    busca_local: str,
    midias_usadas: set[Path],
    grupo_t12: str,
) -> Path:
    chave_busca = _chave_busca_local(busca_local)
    usadas = _caminhos_resolvidos(midias_usadas)
    candidatos = [
        path
        for path in _midias_locais_disponiveis(diretorio_origem)
        if chave_busca in _chave_busca_local(path.stem) and path.resolve() not in usadas
    ]
    if not candidatos:
        raise FileNotFoundError(
            "Template 12 nao possui asset local alternativo para busca_local "
            f"'{busca_local}' no grupo {grupo_t12}. "
            "Adicione uma midia fisica distinta ou corrija as sub-cenas."
        )
    return random.choice(candidatos)


def _chave_busca_local(busca: str) -> str:
    return "_".join(re.findall(r"[a-z0-9]+", normalizar_ascii(busca)))


def _tokens_lexicos(prompt: str) -> frozenset[str]:
    return frozenset(
        token
        for token in re.findall(r"[a-z0-9]+", normalizar_ascii(prompt))
        if token not in STOPWORDS_LEXICAS_INGLES
    )


def _calcular_similaridade_lexica(prompt1: str, prompt2: str) -> float:
    """Mede sobreposicao de termos sem depender da ordem da frase."""

    tokens_1 = _tokens_lexicos(prompt1)
    tokens_2 = _tokens_lexicos(prompt2)
    if not tokens_1 or not tokens_2:
        return 0.0
    return len(tokens_1 & tokens_2) / len(tokens_1 | tokens_2)


def _diversificar_busca_pexels_t12(
    *,
    busca: str,
    cena: dict[str, Any],
    indice_cena: int,
    ...
    prompts_anteriores = _prompts_pexels_t12(estado_t12, grupo_t12)
    maior_similaridade = max(
        (_calcular_similaridade_lexica(busca, anterior) for anterior in prompts_anteriores),
        default=0.0,
    )
    if maior_similaridade <= LIMIAR_SIMILARIDADE_PEXELS_T12:
        return busca

    topico = str(cena.get("topico") or "distinct scene").strip()
    busca_diversificada = f"{busca} {topico} cinematic b-roll"
    logger.warning(
        "Horizontal: cena %s T12 grupo=%s tem busca Pexels redundante "
        "(similaridade=%.0f%%); usando fallback '%s'.",
        indice_cena,
        grupo_t12,
        maior_similaridade * 100,
        busca_diversificada,
    )
    return busca_diversificada
...
def _prompt_google_flow_por_slot(cena: dict[str, Any], index: int) -> str | None:
    """Obtém a instrução externa opcional sem tornar o asset ausente invisível."""

    value = _valor_por_slot(cena.get("prompt_google_flow"), index)
    prompt = str(value or "").strip()
    return prompt or None


def _marcar_slot_como_local(cena: dict[str, Any], index: int) -> None:
    """Persiste a fonte que foi efetivamente escolhida pela curadoria do Studio."""

    fonte = cena.get("fonte_midia")
    indice_cena = int(cena.get("indice") or 0)
    if indice_cena <= 0:
        raise ValueError("Cena sem índice ao registrar mídia local selecionada.")
    tag_local = f"cena_{indice_cena:02d}_local"
    total = TEMPLATE_MEDIA_COUNTS[_template_id(cena)]
    if total > 1:
        fontes = list(fonte) if isinstance(fonte, list) else [fonte] * total
        while len(fontes) < total:
            fontes.append(fonte)
        fontes[index] = "local"
        cena["fonte_midia"] = fontes
        buscas = cena.get("buscas_locais")
        if not isinstance(buscas, list):
            buscas = [None] * total
        while len(buscas) < total:
            buscas.append(None)
        buscas[index] = tag_local
        cena["buscas_locais"] = buscas
    else:
        cena["fonte_midia"] = "local"
        cena["busca_local"] = tag_local
...
def _nome_arquivo_prompt_google_flow(indice_cena: int, letra: str | None) -> str:
    slot = f"_{letra}" if letra else ""
    return f"cena_{indice_cena:02d}{slot}_GOOGLE_FLOW.txt"
...
def _midias_locais_do_manifesto() -> tuple[Path, ...]:
    """Retorna somente a curadoria local explicitamente escolhida no Studio."""

    manifesto = os.environ.get("SYNTHREEL_PROJECT_MANIFEST", "").strip()
    if not manifesto:
        return ()
    try:
        payload = json.loads(Path(manifesto).read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return ()
    valores = payload.get("local_media", []) if isinstance(payload, dict) else []
    if not isinstance(valores, list):
        return ()
    paths = [Path(valor).resolve() for valor in valores if isinstance(valor, str)]
    return tuple(path for path in paths if path.is_file() and path.suffix.lower() in LOCAL_MEDIA_EXTENSIONS)


def _diversificar_templates_do_manifesto() -> bool:
    manifesto = os.environ.get("SYNTHREEL_PROJECT_MANIFEST", "").strip()
    if not manifesto:
        return False
    try:
        ...
        return False
    politica = payload.get("visual_policy", {}) if isinstance(payload, dict) else {}
    return isinstance(politica, dict) and bool(politica.get("diversify_consecutive_templates"))


def _diversificar_templates_consecutivos(cenas: list[dict[str, Any]], logger: Any) -> None:
    """Alterna apenas layouts equivalentes em slots repetidos consecutivos."""

    alternativas = {1: 2, 2: 1, 3: 10, 10: 3, 6: 8, 8: 6, 7: 9, 9: 7}
    anterior: int | None = None
    for cena in cenas:
        atual = _template_id(cena)
        if atual == anterior and atual in alternativas:
            substituto = alternativas[atual]
            cena["template_id"] = substituto
            logger.info(
                "Horizontal: cena %s template %s alternado para %s para evitar repetição consecutiva.",
                cena.get("indice", "?"), atual, substituto,
            )
            atual = substituto
        anterior = atual


def _escolher_midia_local_prioritaria(
    candidatas: tuple[Path, ...],
    usadas: set[Path],
) -> Path | None:
    usadas_resolvidas = _caminhos_resolvidos(usadas)
...
def _normalizar_contrato_subcenas(
    ...
    for indice_mae, cena_mae in enumerate(cenas_brutas, start=1):
        ...
        # T12 never inherits a visual slot from its parent.  Each topic is a
        # separate visual scene and must bring its own query/tag below.
        for chave in (
            "busca_local",
            "buscas_locais",
            "prompt_ou_busca",
            ...
            base.pop(chave, None)
        # Marcador interno de preparo: agrupa apenas as sub-cenas desta cena
        # mae para impedir repeticao visual sem transferir estado ao renderer.
        grupo_t12 = f"t12_{uuid4().hex[:8]}"
        base["t12_group_id"] = grupo_t12
        ...
        for indice_subcena, sub_cena in enumerate(sub_cenas, start=1):
            ...
            fonte_midia = str(cena_mae.get("fonte_midia", "")).strip().lower()
            campo_midia = "busca_local" if fonte_midia == "local" else "prompt_ou_busca"
            valor_midia = sub_cena.get(campo_midia)
            if not isinstance(valor_midia, str) or not valor_midia.strip():
                raise ValueError(
                    f"cena {indice_mae} sub_cena {indice_subcena}: template 12 exige "
                    f"'{campo_midia}' proprio para a mídia exclusiva do tópico."
                )
            ...
            cena_plana["topico"] = topico.strip()
