...
from backend.src.scripts import renderizar_horizontal as renderer
from backend.src.utils import theme_lock
...
class SceneRetentionTests(unittest.TestCase):
    def test_common_scene_with_exactly_four_seconds_is_allowed(self) -> None:
    ...
    def test_common_scene_above_four_seconds_fails_with_retention_context(self) -> None:
        cena = renderer.CenaTemporizada(
            _cena_auditada(7, "Texto oficial curto."),
            inicio=0.0,
            fim_fala=4.0,
        ...
        with self.assertRaisesRegex(
            ...
            r"Cena 07.*4\.001s.*retencao do YouTube",
        ):
            renderer.validar_duracao_maxima_cenas([cena])

    def test_template_twelve_allows_six_seconds_but_rejects_more(self) -> None:
        cena_t12 = renderer.CenaAuditada(
            indice=12,
            template_id=12,
            texto="Texto oficial curto.",
            textos_tela=("Topico",),
            midias=(),
        )
        permitida = renderer.CenaTemporizada(
            cena_t12,
            inicio=0.0,
            fim_fala=5.9,
            fim=6.0,
        ...
        renderer.validar_duracao_maxima_cenas([permitida])

        excedida = renderer.CenaTemporizada(
            cena_t12,
            inicio=0.0,
            fim_fala=6.0,
            fim=6.001,
        ...
        with self.assertRaisesRegex(renderer.TempoCenaExcedidoError, r"template 12: 6\.000s"):
            renderer.validar_duracao_maxima_cenas([excedida])
...
class FFmpegGraphTests(unittest.TestCase):
    ...
    def test_global_graph_has_dynamic_visual_and_audio_transitions(self) -> None:
        ...
        self.assertIn(
            "asetpts=PTS-STARTPTS,volume=0.25,afade=t=in:st=0:d=0.050,"
            "afade=t=out:st=0.750:d=0.050,adelay=734:all=1",
            graph,
        )
        ...
        self.assertIn("alimiter=limit=0.92:attack=5:release=50:latency=1", graph)
    ...
    def test_global_graph_adds_synchronized_writing_sound(self) -> None:
        graph = renderer.construir_filtro_global(
            10.0,
            (),
            eventos_escrita=(renderer.EventoEscrita(inicio=1.2, duracao=0.8),),
        )

        self.assertIn("aevalsrc=exprs=", graph)
        self.assertIn("highpass=f=140,lowpass=f=2600,volume=0.42", graph)
        self.assertIn("adelay=1200:all=1[escrita_audio_0]", graph)
        self.assertIn("amix=inputs=3:duration=longest", graph)
    ...
    def test_scene_command_uses_layout_mapping_order_and_media_loops(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_execute(args, etapa, **kwargs):  # type: ignore[no-untyped-def]
                captured.extend(str(arg) for arg in args)
                self.assertEqual(kwargs, {"timeout": renderer.SCENE_RENDER_TIMEOUT_SECONDS})
        ...
        self.assertNotIn("h264_amf", captured)
        filter_graph = captured[captured.index("-filter_complex") + 1]
        self.assertIn("setrange=tv,format=yuv420p", filter_graph)
        self.assertEqual(captured[captured.index("-color_range") + 1], "tv")
        self.assertEqual(captured[captured.index("-colorspace") + 1], "bt709")
        self.assertEqual(captured[captured.index("-color_primaries") + 1], "bt709")
        self.assertEqual(captured[captured.index("-color_trc") + 1], "bt709")

    def test_worker_subprocess_drains_pipes_with_communicate_timeout(self) -> None:
        processo = Mock()
        processo.returncode = 0
        processo.communicate.return_value = ("saida", "")

        with patch.object(renderer.subprocess, "Popen", return_value=processo) as popen:
            resultado = renderer._executar(
                ["ffmpeg", "-version"],
                "renderizacao da cena 01",
                timeout=renderer.SCENE_RENDER_TIMEOUT_SECONDS,
            )

        self.assertEqual(resultado.stdout, "saida")
        ...
        processo.communicate.assert_called_once_with(
            timeout=renderer.SCENE_RENDER_TIMEOUT_SECONDS
        )

    def test_worker_subprocess_kills_process_after_timeout_and_drains_pipes(self) -> None:
        processo = Mock()
        processo.communicate.side_effect = [
            subprocess.TimeoutExpired("ffmpeg", renderer.SCENE_RENDER_TIMEOUT_SECONDS),
            ("", "diagnostico final"),
        ]

        with self.assertRaisesRegex(RuntimeError, "(?s)Tempo limite.*diagnostico final"):
            with patch.object(renderer.subprocess, "Popen", return_value=processo):
                renderer._executar(
                    ["ffmpeg", "-version"],
                    "renderizacao da cena 01",
                    timeout=renderer.SCENE_RENDER_TIMEOUT_SECONDS,
        ...
        self.assertEqual(
            processo.communicate.call_args_list,
            [
                call(timeout=renderer.SCENE_RENDER_TIMEOUT_SECONDS),
                call(),
            ],
        )
    ...
    def test_final_command_uses_one_input_per_dynamic_transition(self) -> None:
        ...
        self.assertLess(captured.index("-fflags"), captured.index("-i"))
        self.assertEqual(captured[captured.index("-fflags") + 1], "+genpts")
        self.assertEqual(captured[captured.index("-fps_mode") + 1], "cfr")
        self.assertEqual(captured[captured.index("-r") + 1], str(renderer.FPS))
        self.assertEqual(
            captured[captured.index("-video_track_timescale") + 1], "90000"
        )
        self.assertEqual(captured[captured.index("-rc") + 1], "vbr_peak")
        self.assertEqual(captured[captured.index("-b:v") + 1], "8M")
        self.assertEqual(captured[captured.index("-maxrate") + 1], "10M")
        self.assertEqual(captured[captured.index("-bufsize") + 1], "16M")
        self.assertEqual(captured[captured.index("-enforce_hrd") + 1], "1")
        self.assertIn("+faststart", captured)
        self.assertNotIn("-async", captured)
        self.assertNotIn("-vsync", captured)
