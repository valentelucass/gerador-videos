...
from src.scripts.preparar_horizontal import (
...
from src.utils.asset_signature import calcular_hash_hibrido
from src.utils.file_retry import replace_com_retry, rmtree_com_retry, unlink_com_retry
from src.utils.logger import get_logger
from src.utils.text_helpers import normalizar_ascii
from src.utils.theme_lock import TravaTema
...
SCENE_RESUME_MANIFEST_VERSION = 1
...
class TrabalhoRenderizacaoCena:
    ...
    ffmpeg: str
    ffprobe: str
    opcoes_encoder_video: tuple[str, ...]
    fonte: Path | None
    seta_path: Path
...
def _atribuir_fundos_estaticos(
    ...
    for cena in cenas:
        ...
        fundo = _selecionar_fundo_estatico_deterministico(cena, fundos)
        logger.info(
...
def _renderizar_horizontal_sem_lock(
    ...
    *,
    ...
    cenas = _atribuir_fundos_estaticos(cenas, ffprobe)
    ...
    try:
        ...
        opcoes_encoder_video = _validar_recursos_ffmpeg(ffmpeg, run_dir)
        logger.info(
            "Horizontal: encoder de video selecionado: %s",
            _nome_encoder_video(opcoes_encoder_video),
        )
        ...
        trabalhos_cenas = tuple(
            TrabalhoRenderizacaoCena(
                ...
                clip_path=run_dir
                / f"cena_{cena_temporizada.cena.indice:02d}_render.mp4",
                ...
                ffmpeg=ffmpeg,
                ffprobe=ffprobe,
                opcoes_encoder_video=opcoes_encoder_video,
                fonte=fonte,
                seta_path=seta.path,
            )
...
def _executar(args: Sequence[str], etapa: str) -> subprocess.CompletedProcess[str]:
...
def validar_duracao_maxima_cenas(
    ...
    """Rejects acoustically timed scenes that exceed the AVD ceiling."""

    for cena_temporizada in cenas_temporizadas:
        duracao = cena_temporizada.duracao
        if duracao > MAX_SCENE_DURATION:
            ...
            raise TempoCenaExcedidoError(
                ...
                f"(maximo permitido: {MAX_SCENE_DURATION:.3f}s)."
            )
    ...
    *,
    ...
    args.extend(
        [
            ...
            "-fps_mode",
            "cfr",
            *(opcoes_encoder_video or _opcoes_libx264()),
            str(output_path),
        ]
    )
    _executar(args, f"renderizacao da cena {cena.indice:02d}")
...
def _processar_trabalho_cena(
    ...
    try:
        ...
        _renderizar_cena(
            ...
            opcoes_encoder_video=trabalho.opcoes_encoder_video,
            fonte=trabalho.fonte,
            seta_path=trabalho.seta_path,
        )
...
def construir_filtro_global(
    ...
    *,
    ...
    for indice, transicao in enumerate(transicoes):
        ...
        if transicao.duracao_audio is not None:
            ...
            filtros.append(
                ...
                f"atrim=duration={_fmt_float(transicao.duracao_audio)},"
                f"asetpts=PTS-STARTPTS,volume=3.0,"
    ...
    filtros.append(
        ...
        f"dropout_transition=0:normalize=0,alimiter=limit=0.95:latency=1,"
...
def _renderizar_compilacao_final(
    *,
    ...
    args.extend(
        [
            # Force the concat demuxer output into a single CFR timeline before
            # it reaches the global filter.  Without this, a sequence that
            # mixes generated image clips and native video may end at the last
            # timestamp inferred by the demuxer instead of the manifest total.
            "-async",
            "1",
            "-vsync",
            "1",
