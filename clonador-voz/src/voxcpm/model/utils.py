...
def pick_runtime_dtype(device: str, configured_dtype: str) -> str:
    ...
    badcase detector triggers infinite retries. On CPU, low precision kernels
    can produce silent output on some Windows/PyTorch builds. float32 is the
    stable fallback for both CPU and MPS today. CUDA keeps whatever the
    ...
    override_name = "VOXCPM_MPS_DTYPE" if device == "mps" else "VOXCPM_CPU_DTYPE"
    override = os.environ.get(override_name, "").strip().lower()
    if override:
        if override not in _VALID_DTYPE_OVERRIDES:
            raise ValueError(
                f"{override_name}='{override}' is not one of "
